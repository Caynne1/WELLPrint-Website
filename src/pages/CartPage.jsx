    ...
    >
      ...
      >
        {item.thumbnail_url ? (
          ...
          <ImagePlus
            size={22}
            style={{ color: item.accent || 'var(--wp-green)', opacity: 0.2 }}
          />
        )}
      </div>
  ...
  return (
    <div>
      <label
        ...
        style={{
          color: error ? 'var(--wp-magenta)' : 'rgba(216,216,216,0.5)',
        }}
      >
        {label}
        {required && (
          <span className="ml-1" style={{ color: 'var(--wp-green)' }}>
            *
          </span>
        )}
      </label>
...
const ORDER_STEPS = [
  {
    icon: ClipboardList,
    title: 'Review Cart',
    desc: 'Check your selected products, options, quantity, design submission method, and pickup or delivery choice.',
    color: 'var(--wp-green)',
  },
  {
    icon: MessageSquare,
    title: 'Complete Order Details',
    desc: 'Fill out your name, email, phone number, and any order notes so we can process your request properly.',
    color: 'var(--wp-cyan)',
  },
  {
    icon: Paintbrush,
    title: 'Artwork & Confirmation',
    desc: 'If needed, we review your uploaded file, emailed design, or layout request before final production.',
    color: 'var(--wp-yellow)',
  },
  {
    icon: Printer,
    title: 'Production',
    desc: 'Once confirmed, your order moves into processing and printing based on the chosen product and timeline.',
    color: 'var(--wp-magenta)',
  },
  {
    icon: PackageCheck,
    title: 'Pickup or Delivery',
    desc: 'Your order is quality-checked, packed, and prepared for pickup or delivery to your location.',
    color: 'var(--wp-green)',
  },
]
...
function CheckoutModal({ open, onClose, cart, totalPrice, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      let uploadSourceFile = form.file || null

      if (!uploadSourceFile) {
        const itemWithFile = cart.find((item) => item.designFile instanceof File)
        if (itemWithFile) uploadSourceFile = itemWithFile.designFile
      }
      ...
      if (uploadSourceFile) {
        ...
        const itemWithFileName = cart.find((item) => item.designFileName)
        if (itemWithFileName) {
          uploadedFileName = itemWithFileName.designFileName
        }
      }
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center justify-center text-center py-16 px-2">
              <div
                ...
                style={{
                  background: 'rgba(19,161,80,0.12)',
                  border: '1px solid rgba(19,161,80,0.3)',
                }}
              >
              ...
              <p className="text-ivory-300/55 text-sm max-w-sm leading-relaxed mb-3">
                Thanks, <span className="text-white font-semibold">{form.name}</span>! Your
                order has been submitted successfully.
              </p>

              {placedOrderId && (
                <div
                  className="px-4 py-3 rounded-sm border mb-6"
                  style={{
                    background: 'rgba(255,255,255,0.04)',
                    borderColor: 'rgba(255,255,255,0.08)',
                  }}
                >
                  <div className="text-[10px] uppercase tracking-[0.2em] text-ivory-300/30 mb-1">
                    Order Reference
                  </div>
              ...
              <p className="text-ivory-300/35 text-xs max-w-xs leading-relaxed mb-8">
                Please save this reference for now. We will use this later for order tracking.
              </p>

              <button onClick={onClose} className="btn-press text-sm">
                Close
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
              <div>
                <p className="text-ivory-300/40 text-xs leading-relaxed">
                  Fill out your details below to complete your order. We already
                  received your selected cart items automatically.
                </p>
              </div>
              ...
              <Field label="Attach Reference File (optional)">
                ...
                >
                  ...
                  <span
                    className="ml-auto shrink-0 btn-press-outline"
              ...
              <div className="bg-ink-800 border border-white/[0.07] rounded-sm p-4">
                <div className="font-body text-[10px] tracking-widest uppercase text-ivory-300/30 mb-3">
                  Cart snapshot
                </div>
                ...
                <div className="space-y-2">
                  {cart.map((item) => (
                    <div key={item.key} className="flex items-center justify-between text-xs gap-4">
                      <span className="text-ivory-300/55 truncate max-w-[220px]">
                        {item.name}
                      </span>
                      <span className="text-ivory-300/40 font-body shrink-0">
                        {item.qty} {item.unit || 'pcs'} —{' '}
                        {formatPHP(
                          (item.unitPrice || 0) * (item.qty || 0) + (item.deliveryFee || 0)
                        )}
                      </span>
                    </div>
                  ))}
                ...
                <div className="mt-3 pt-3 border-t border-white/[0.06] flex justify-between items-center">
                  <span className="text-ivory-300/40 text-xs font-body">
                    Estimated Total
                  </span>
                  <span
                ...
                <p className="text-[10px] font-body text-ivory-300/20 mt-1">
                  Final price may still be confirmed after review.
                </p>
              </div>
                ...
                <div className="flex items-center justify-between w-full gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={onClose}
                      className="btn-press-ghost text-sm"
                    >
                      Cancel
                    </button>
              ...
              <div className="grid lg:grid-cols-3 gap-10">
                    ...
                    {!showClearConfirm ? (
                      ...
                      <div className="flex items-center gap-3 text-[10px] font-body">
                        ...
                        <button
                          ...
                          className="text-wp-magenta hover:underline"
                        >
                          Yes, clear
                        </button>
                        <button
                          onClick={() => setShowClearConfirm(false)}
                          className="text-ivory-300/40 hover:text-white"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  <div className="mt-6 bg-ink-800 border border-white/[0.07] rounded-sm p-5 flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-ivory-300/45 text-xs leading-relaxed">
                        If you selected design upload, email submission, or layout assistance,
                        our team will review that together with your order details after checkout.
                      </p>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-1 space-y-4">
                  <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-body text-[10px] tracking-widest uppercase text-wp-green">
                          Order Summary
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-5 space-y-3">
                        ...
                        return (
                          ...
                          >
                            <div className="text-ivory-300/50 leading-snug flex-1 min-w-0">
                              ...
                              <span className="text-ivory-300/25 font-body">
                                {item.qty} {item.unit || 'pcs'} × {formatPHP(item.unitPrice || 0)}
                              </span>

                              {item.deliveryFee > 0 && (
                                <span className="block text-ivory-300/20 font-body mt-1">
                                  + Delivery {formatPHP(item.deliveryFee)}
                                </span>
                              )}
                            </div>

                            <span className="text-ivory-300/70 font-body shrink-0">
                              {formatPHP(rowTotal)}
                            </span>
                          </div>
                        )
                      ...
                      <div className="border-t border-white/[0.08] pt-3 mt-3">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-ivory-300/70 text-xs font-body">
                            {formatPHP(totalPrice)}
                          </span>
                        </div>

                        <div className="flex items-baseline justify-between pt-3 border-t border-white/[0.08] mt-3">
                        ...
                        <p className="text-ivory-300/20 text-[9px] font-body mt-1.5 leading-relaxed">
                          Final amount may still be confirmed after review.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-ink-800 border border-white/[0.07] rounded-sm p-4 space-y-2.5">
                    {[
                      {
                        text: 'Color accuracy guaranteed',
                        color: 'var(--wp-green)',
                      },
                      {
                        text: 'Reprint if we make an error',
                        color: 'var(--wp-green)',
                      },
                      {
                        text: 'Backed by Espiel-Bereso Group',
                        color: 'var(--wp-cyan)',
                      },
                    ].map(({ text, color }) => (
                      <div
                        key={text}
                        className="flex items-center gap-2.5 text-xs text-ivory-300/45"
                      >
                        <CheckCircle
                          size={12}
                  ...
                  <div className="bg-ink-800 border border-white/[0.07] rounded-sm p-4 space-y-3">
                    <div className="font-body text-[10px] tracking-widest uppercase text-ivory-300/30 mb-1">
                      Need help? Contact us
                    </div>

                    <a
                      href="tel:+6328XXXXXXXX"
                      className="flex items-center gap-3 text-xs text-ivory-300/55 hover:text-wp-green transition-colors"
                    >
                      <Phone size={13} style={{ color: 'var(--wp-green)' }} /> +63
                      (2) 8XXX-XXXX
                    </a>

                    <a
                      href="mailto:hello@wellprint.com.ph"
                      className="flex items-center gap-3 text-xs text-ivory-300/55 hover:text-wp-cyan transition-colors"
                    >
                      <Mail size={13} style={{ color: 'var(--wp-cyan)' }} />{' '}
                    ...
                    <div key={step.title} className="relative">
                      {i < ORDER_STEPS.length - 1 && (
                        <div
                          ...
                          style={{
                            background:
                              'linear-gradient(90deg, rgba(19,161,80,0.3), transparent)',
                          }}
                        />
                      )}
                      ...
                      <div className="bg-ink-800 border border-white/[0.07] rounded-sm p-5 h-full relative z-10">
                        <div
                          className="font-body text-[9px] tracking-widest mb-3"
                        ...
                        <div
                          className="font-semibold text-white text-sm mb-2 leading-snug"
                          style={{ fontFamily: "'Lora', serif" }}
                        >
                          {step.title}
                        </div>
