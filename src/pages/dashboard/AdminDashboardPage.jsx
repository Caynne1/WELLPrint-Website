...
function SparkBars({ data = [], color = '#2DB04B' }) {
...
function SystemHealthDot({ status }) {
  const colors = { ok: '#2DB04B', warn: '#FBB03B', error: '#EC008C' }
  return (
  ...
  return (
    <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
      <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center gap-2"
        ...
        <Server size={11} style={{ color: '#29ABE2' }} />
        <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: '#29ABE2' }}>System Health</span>
        <span className="ml-auto font-mono text-[9px] text-ivory-300/25">Admin Only</span>
      ...
      <div className="p-4 space-y-2.5">
        {checks.map(c => (
          <div key={c.label} className="flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] text-ivory-300/60 flex-1">{c.label}</span>
            <span className="font-mono text-[9px] text-ivory-300/30">{c.note}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
      <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center gap-2"
        ...
        <Target size={11} style={{ color: '#FBB03B' }} />
        <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: '#FBB03B' }}>Monthly Target</span>
        <span className="ml-auto font-mono text-[9px] text-ivory-300/25">Admin Only</span>
        <Lock size={9} className="text-ivory-300/20" />
      </div>
      <div className="p-4">
        <div className="flex items-end justify-between mb-2">
          <div>
            <div className="text-white text-xl font-black" style={{ fontFamily: "'Playfair Display', serif" }}>
              {pct}%
            </div>
            <div className="text-ivory-300/40 text-[10px] font-mono">of {formatPHP(monthlyTarget)}</div>
          </div>
          <div className="text-right">
            <div className="text-white text-sm font-bold">{formatPHP(completedRev)}</div>
            <div className="text-ivory-300/30 text-[10px] font-mono">{formatPHP(remaining)} to go</div>
          </div>
        </div>
        <div className="h-2 rounded-full overflow-hidden mb-2" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <div className="h-full rounded-full transition-all duration-700"
            style={{
              width: `${pct}%`,
              background: pct >= 100 ? '#2DB04B' : pct >= 60 ? '#FBB03B' : '#EC008C',
              boxShadow: `0 0 8px ${pct >= 100 ? '#2DB04B' : pct >= 60 ? '#FBB03B' : '#EC008C'}40`
            }} />
        </div>
        <div className="text-ivory-300/25 text-[9px] font-mono">
...
function AuditLogPanel() {
  const logs = [
    { user: 'admin',      action: 'Updated order #ORD-0047 → Printing', time: '4m ago',  color: '#2DB04B' },
    { user: 'maria.s',   action: 'Viewed order #ORD-0046',              time: '11m ago', color: '#29ABE2' },
    { user: 'admin',      action: 'Added product "Tarpaulin A3"',       time: '1h ago',  color: '#FBB03B' },
    { user: 'juan.d',    action: 'Logged in',                           time: '2h ago',  color: '#29ABE2' },
    { user: 'admin',      action: 'Edited staff permissions for maria.s', time: '3h ago', color: '#EC008C' },
  ]
  return (
    <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
      <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center gap-2"
        style={{ background: 'rgba(236,0,140,0.04)' }}>
        <Activity size={11} style={{ color: '#EC008C' }} />
        <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: '#EC008C' }}>Audit Log</span>
        <span className="ml-auto font-mono text-[9px] text-ivory-300/25">Admin Only</span>
        <Lock size={9} className="text-ivory-300/20" />
      </div>
        ...
        {logs.map((l, i) => (
          <div key={i} className="flex items-start gap-3 px-4 py-2.5">
            <div className="w-5 h-5 rounded-sm shrink-0 flex items-center justify-center text-[8px] font-bold mt-0.5"
              style={{ background: `${l.color}14`, color: l.color, border: `1px solid ${l.color}25`, fontFamily: "'Playfair Display', serif" }}>
              {l.user.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-mono text-[9px]" style={{ color: l.color }}>@{l.user}</div>
              <div className="text-ivory-300/55 text-[10px] truncate">{l.action}</div>
            </div>
            <span className="font-mono text-[9px] text-ivory-300/25 shrink-0">{l.time}</span>
          </div>
        ))}
      </div>
    </div>
...
function StaffActivityPanel() {
  const staff = [
    { name: 'Maria Santos',  username: 'maria.s',  role: 'staff', status: 'online',  lastAction: 'Viewing Orders', color: '#2DB04B' },
    { name: 'Juan Dela Cruz', username: 'juan.d',  role: 'staff', status: 'idle',    lastAction: 'Last seen 12m ago', color: '#FBB03B' },
    { name: 'Ana Reyes',     username: 'ana.r',    role: 'staff', status: 'offline', lastAction: 'Last seen 2h ago', color: '#555' },
  ]
  const statusColors = { online: '#2DB04B', idle: '#FBB03B', offline: '#444' }
  return (
    <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
      <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center gap-2"
        style={{ background: 'rgba(45,176,75,0.04)' }}>
        <Users size={11} style={{ color: 'var(--wp-green)' }} />
        <span className="font-mono text-[10px] tracking-widest uppercase text-wp-green">Staff Activity</span>
        <span className="ml-auto font-mono text-[9px] text-ivory-300/25">Admin Only</span>
        <Lock size={9} className="text-ivory-300/20" />
      </div>
      <div className="divide-y divide-white/[0.04]">
          ...
          <div key={i} className="flex items-center gap-3 px-4 py-3">
            <div className="relative shrink-0">
              <div className="w-7 h-7 rounded-sm flex items-center justify-center text-xs font-bold"
                style={{ background: `${statusColors[s.status]}14`, border: `1px solid ${statusColors[s.status]}30`, color: statusColors[s.status], fontFamily: "'Playfair Display', serif" }}>
                {s.name.charAt(0)}
              </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-ivory-300/35 text-[9px] font-mono truncate">{s.lastAction}</div>
            </div>
            <span className="font-mono text-[9px] capitalize px-2 py-0.5 rounded-sm"
...
export default function AdminDashboardPage() {
  ...
  const STATS = [
    { label: 'New Inquiries', value: newOrders,               icon: Inbox,       color: '#EC008C', note: 'Awaiting response' },
    { label: 'Active Orders', value: activeOrders,            icon: Clock,       color: '#FBB03B', note: 'In progress' },
    { label: 'Completed',     value: completedOrders,         icon: CheckCircle, color: '#2DB04B', note: 'All time' },
    { label: 'Est. Revenue',  value: formatPHP(totalRevenue), icon: TrendingUp,  color: '#29ABE2', note: 'Completed only' },
  ]

  const QUICK_ACTIONS = [
    { label: 'All Orders',  to: '/dashboard/orders',    icon: Package,    color: '#EC008C' },
    { label: 'Products',    to: '/dashboard/products',  icon: ShoppingBag,color: '#FBB03B' },
    { label: 'Analytics',   to: '/dashboard/analytics', icon: BarChart2,  color: '#29ABE2' },
    ...(isAdmin ? [{ label: 'Staff', to: '/dashboard/staff', icon: Users, color: '#2DB04B' }] : []),
  ]
    ...
    <AdminLayout>
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-white text-2xl font-bold mt-0.5" style={{ fontFamily: "'DM Serif Display', serif" }}>
            ...
            {isAdmin && (
              <span className="ml-2 inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-sm align-middle"
                style={{ background: 'rgba(236,0,140,0.1)', border: '1px solid rgba(236,0,140,0.2)', color: '#EC008C', verticalAlign: 'middle' }}>
                <Shield size={9} /> Admin
              </span>
            )}
          </h1>
          <p className="text-ivory-300/40 text-sm mt-0.5">
            ...
            {weekTrend !== 0 && (
              <span className="ml-2 text-[10px] font-mono" style={{ color: weekTrend > 0 ? '#2DB04B' : '#EC008C' }}>
        ...
        {STATS.map(({ label, value, icon: Icon, color, note }) => (
          <div key={label} className="bg-ink-800 border border-white/[0.07] rounded-sm p-5 flex flex-col gap-3 hover:border-white/[0.12] transition-all">
            ...
            <div>
              ...
              <div className="text-ivory-300/25 text-[10px] font-mono mt-0.5">{note}</div>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="lg:col-span-2 bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between" style={{ background: 'rgba(45,176,75,0.04)' }}>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-wp-green" style={{ animation: 'pulse 2s infinite' }} />
              <span className="font-mono text-[10px] tracking-widest uppercase text-wp-green">Live Orders</span>
            </div>
          ...
          {recentOrders.length === 0 ? (
            <div className="py-16 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-ivory-300/30 text-xs font-mono">No orders yet — they'll appear here in real time</p>
            </div>
          ) : (
            <div className="divide-y divide-white/[0.04]">
                    ...
                    <div className="min-w-0 flex-1 grid sm:grid-cols-3 gap-1 items-center">
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {order.status === 'new' && <span className="text-[8px] font-mono px-1.5 py-0.5 rounded-sm" style={{ background: 'rgba(236,0,140,0.2)', color: '#EC008C' }}>NEW</span>}
                        </div>
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded-sm mt-0.5 inline-block" style={{ background: st.bg, color: st.color }}>{st.label}</span>
                      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-white/[0.06]" style={{ background: 'rgba(41,171,226,0.04)' }}>
              <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: '#29ABE2' }}>Order Breakdown</span>
            </div>
            <div className="p-4">
              {orders.length === 0 ? (
                <p className="text-center text-ivory-300/25 text-xs font-mono py-6">No data yet</p>
              ) : (
                <>
                  <div className="flex items-center justify-center mb-4">
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-white font-black text-lg leading-none" style={{ fontFamily: "'Playfair Display', serif" }}>{orders.length}</span>
                        <span className="text-ivory-300/30 text-[9px] font-mono">total</span>
                      </div>
                    </div>
                  </div>
                    ...
                    {donutSegments.map(seg => (
                      <div key={seg.label} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-mono" style={{ color: seg.color }}>{seg.label}</span>
                        </div>
                        <span className="text-[10px] font-mono text-ivory-300/40">{seg.value}</span>
                      </div>
                    ))}
                  </div>
                </>
      ...
      {isAdmin && (
        <>
          <div className="flex items-center gap-3 mb-4 mt-2">
            <div className="flex items-center gap-2">
              <Shield size={12} style={{ color: '#EC008C' }} />
              <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: '#EC008C' }}>Admin Controls</span>
            </div>
      ...
      <div className="mt-4 bg-ink-800 border border-white/[0.07] rounded-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.06]" style={{ background: 'rgba(45,176,75,0.04)' }}>
          <span className="font-mono text-[10px] tracking-widest uppercase text-wp-green">7-Day Activity</span>
        </div>
        <div className="p-4">
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center">
                  <div className="w-full rounded-sm"
                    style={{
                      height: `${h}%`,
                      background: isToday ? 'var(--wp-green)' : 'rgba(45,176,75,0.22)',
                      boxShadow: isToday ? '0 0 8px rgba(45,176,75,0.4)' : 'none',
                    }} />
                </div>
              )
            })}
          ...
          <div className="flex gap-1">
            {last7.map((d, i) => (
              <div key={i} className="flex-1 text-center text-[9px] font-mono text-ivory-300/25">{d.label}</div>
            ))}
          </div>
        </div>
      </div>
