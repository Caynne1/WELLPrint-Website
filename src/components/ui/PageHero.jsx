export default function PageHero({ label, title, titleAccent, subtitle, children }) {
  return (
    <section className="relative bg-ink-950 pt-36 pb-16 overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[70vw] h-64 bg-wp-green/[0.04] blur-[80px] pointer-events-none" aria-hidden="true" />
      {/* CMYK bar top */}
      <div className="absolute top-0 left-0 right-0 cmyk-bar" aria-hidden="true" />

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex items-center gap-3 mb-5">
          <div className="h-px w-10 bg-wp-green" />
          <span className="font-mono text-[10px] tracking-[0.25em] uppercase" style={{ color: 'var(--wp-green)' }}>
            {label}
          </span>
        </div>
        <h1
          className="text-white mb-4 leading-tight"
          style={{
            fontFamily: "'Playfair Display', serif",
            fontWeight: 900,
            fontSize: 'clamp(2.2rem, 5vw, 4rem)',
          }}
        >
          {title}
          {titleAccent && (
            <> <span className="italic" style={{ color: 'var(--wp-green)' }}>{titleAccent}</span></>
          )}
        </h1>
        {subtitle && (
          <p className="text-ivory-300/60 text-lg max-w-2xl leading-relaxed">{subtitle}</p>
        )}
        {children}
      </div>
    </section>
  )
}
